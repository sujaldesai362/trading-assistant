...
def get_options_chain(symbol: str, expiry: Optional[str] = None, option_type: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if option_type and contract_type != option_type.lower():
            continue
        if expiry and expiration_date != expiry:
            continue
    ...
    if direction == "bullish":
        ...
        for long_leg in longs:
            ...
            for short_leg in shorts[:4]:
                ...
                rr = max_profit / debit
                score = ((score_contract(long_leg) + score_contract(short_leg)) / 2) + rr * 0.15
...
def build_fallback_trade(symbol: str, stock_price: float, expiry: Optional[str], option_type: Optional[str]) -> Dict[str, Any]:
    desired_type = (option_type or "call").lower()
    direction = "bullish" if desired_type == "call" else "bearish"

    strike_buy = round(stock_price)
    strike_sell = strike_buy + 3 if desired_type == "call" else strike_buy - 3

    if desired_type == "call":
        strategy = "bull_call_spread"
        legs = [
            {"action": "buy", "strike": strike_buy, "type": "call"},
            {"action": "sell", "strike": strike_sell, "type": "call"},
        ]
    else:
        strategy = "bear_put_spread"
        legs = [
            {"action": "buy", "strike": strike_buy, "type": "put"},
            {"action": "sell", "strike": strike_sell, "type": "put"},
        ]
    ...
    return {
        "best_trade": {
            ...
            "legs": legs,
            "estimated_entry": "Manual check needed for live mid-price",
            "position_size": 1,
            "max_loss": 200,
            "max_profit": 100,
            "confidence_score": 0.45,
            "why_it_works": f"Fallback mode: live stock price is {stock_price:.2f}, but options chain snapshot returned no data.",
            "what_could_kill_it": "No live contract-quality checks were available, so this is lower confidence.",
        },
        "backup_trade": None,
...
def build_real_trade(symbol: str, expiry: Optional[str], option_type: Optional[str]) -> Dict[str, Any]:
    ...
    if not chain:
        return build_fallback_trade(symbol, stock_price, expiry, option_type)
...
def scan_watchlist(option_type: Optional[str] = None, max_results: int = 3):
    ...
    for ticker in watchlist:
        ...
        if trade.get("best_trade"):
            ...
            rejected.append({
                "ticker": ticker,
                "reason": trade.get("no_trade_reason", "Unknown reason")
            })
...
@app.get("/debug/chain")
def debug_chain(symbol: str, expiry: Optional[str] = None, option_type: Optional[str] = None):
    chain = get_options_chain(symbol, expiry=expiry, option_type=option_type)
    filtered = filter_candidates(chain, (option_type or "call").lower()) if chain else []
    return {
        "symbol": symbol,
        "raw_count": len(chain),
